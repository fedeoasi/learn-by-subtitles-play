...
class ElasticSearchSpec extends FunSpec with Matchers with Logging {
  ...
  describe("Elastic Search") {
    ...
    describe("Subtitle indexing") {
      it("should be able to index an entry of a subtitle file") {
        withInteractorAndIndex {
          case (interactor, name) =>
            val entry = SubEntry(1, format.parse("00:15:11,123"), format.parse("00:16:11,123"), "This is the content")
            interactor.indexSubtitleEntry(name, entry, "subId", "imdbId", flush = true)
      ...
      it("should be able to index an entire subtitle file entry by entry") {
        withInteractorAndIndex {
          case (interactor, name) =>
            val input = fromFile("resources/four-entries.srt").getLines().mkString("\n")
      ...
      it("should be able to index an entire subtitle file") {
        withInteractorAndIndex {
          case (interactor, name) =>
            val fileString = Utils.readFileIntoString("resources/four-entries.srt")
            interactor.indexSubtitleContent(name, fileString, "subId", "imdbId", flush = true)
      ...
      it("should be able to index and search two subtitle files") {
        withInteractorAndIndex {
          case (interactor, name) =>
            val file1String = Utils.readFileIntoString("resources/four-entries.srt")
            val file2String = Utils.readFileIntoString("resources/other.srt")
            interactor.indexSubtitleContent(name, file1String, "subId1", "imdbId1", flush = true)
            interactor.indexSubtitleContent(name, file2String, "subId2", "imdbId2", flush = true)
      ...
      it("should allow the same subtitle to be indexed twice") {
        withInteractorAndIndex {
          case (interactor, name) =>
            ...
            val file2String = Utils.readFileIntoString("resources/other.srt")
            interactor.indexSubtitleContent(name, file1String, "subId1", "imdbId1", flush = true)
            interactor.indexSubtitleContent(name, file2String, "subId2", "imdbId1", flush = true)
  ...
  def withInteractorAndIndex(test: (ElasticSearchInteractor, String) => Unit): Unit = {
    val interactor = new ElasticSearchInteractor()
