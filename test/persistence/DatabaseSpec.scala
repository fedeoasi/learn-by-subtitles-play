...
class DatabaseSpec extends FunSpec with Matchers with WithPersistenceManager with OptionValues {
  ...
  describe("Movies") {
    ...
    it("should delete a movie") {
      val movie =
        Movie("toDelete", 2013, "testMovie", "http://something.jpg", None)
      persistenceManager.saveMovie(movie)
      val returnedMovie = persistenceManager.findMovieById(movie.imdbID)
      returnedMovie.value.copy(id = None) should be(movie)
  ...
  describe("Subtitles") {
    ...
    it("should be able to save a subtitle") {
      val movie =
        Movie("testMovie", 2012, "testMovie", "http://something.jpg", None)
      persistenceManager.saveMovie(movie)
      val subtitle = Subtitle("abc", "testMovie")
      persistenceManager.saveSubtitle(subtitle)
      val returnedMovie =
        persistenceManager.findSubtitleForMovie(subtitle.imdbId)
    ...
    it("should list the inserted subtitles as non-indexed") {
      val movie =
        Movie("testMovie3", 2013, "testMovie3", "http://something3.jpg", None)
      persistenceManager.saveMovie(movie)
    ...
    it("should list mark a subtitle as indexed") {
      val movie =
        Movie("testMovie4", 2013, "testMovie4", "http://something4.jpg", None)
      persistenceManager.saveMovie(movie)
      val id: String = "abcd"
  ...
  describe("Episodes") {
    ...
    it("should find an episode by series id, season, and episode number") {
      ...
      val returnedMovie =
        persistenceManager.findEpisodeForSeries(series.imdbID, 1, 1)
      returnedMovie should be(Some(episode))
    }
    ...
    it("should list the episodes with no subtitles") {
      ...
      persistenceManager.saveSubtitle(
        Subtitle("idForListingEpisodes", "eimdbId2")
      )
      val episodesToProcess: List[Episode] =
        persistenceManager.findEpisodesWithNoSubtitles()
    ...
    describe("Movies and Episodes") {
      it("should return a map imdbId -> title for the requested imdbIds") {
        val movie =
          Movie("meId", 2013, "testMovie", "http://something.jpg", None)
        val series =
          Series("meId2", 2010, "testSeries", "http://something.jpg", None)
        persistenceManager.saveMovie(movie)
        persistenceManager.saveSeries(series)
    ...
    describe("Subtitle downloads") {
      ...
      it("should find the two latest download errors") {
        ...
        persistenceManager
          .lastKDownloadErrors(2)
          .map(_.subtitleId) shouldBe Seq("sId3", "sId2")
      }
    }
  }
}
