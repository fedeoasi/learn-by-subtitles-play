...
class SrtParser @Inject() () extends Logging {
  ...
  private[parsing] def computeIntervals(
    indices: List[Int],
    offset: Int,
    end: Int
  ): List[(Int, Int)] = {
    ...
    indices
      .foldLeft((emptyIntervals, 0)) { case ((intervals, previousEnd), index) =>
  ...
  private[parsing] def highlightedLines(input: String): List[Int] = {
    parseSrt(getInputStream(input), onlyHighlighted = true, getDelimiter(input))
      .map { se =>
        se.number
      }
  }
  ...
  private def parseSrt(
    input: InputStream,
    onlyHighlighted: Boolean,
    delimiter: String,
    filterByIndex: (Int) => Boolean = _ => true
  ): List[SubEntry] = {
    foldSrt(input, delimiter, (List.empty[SubEntry], 1)) { case ((list, index), line) =>
  ...
  private def countEntries(input: InputStream, delimiter: String): Int = {
    foldSrt(input, delimiter, 0) { case (acc, line) =>
      if (line.matches("^[\r\n]+$")) {
        acc
      } else {
        acc + 1
  ...
  private def readEntry(
    ...
    try {
      ...
      case t: Throwable =>
        logger.error(
          "Exception while processing string: " + string + " " + t
            .printStackTrace()
        )
        None
    }
