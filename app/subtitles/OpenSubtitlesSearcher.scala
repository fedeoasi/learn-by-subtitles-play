...
class OpenSubtitlesSearcher @Inject() (persistenceManager: PersistenceManager)
    extends SubtitleSearcher
    with SeriesDetailProvider
    with Logging {
  ...
  override def getSubtitleCandidates(imdbId: String): Array[Object] = {
    ...
    val response = withTokenCheck(token) { token =>
      client.execute(
        config,
        "SearchSubtitles",
        Array[AnyRef](token, Array(searchParam))
      )
    }
    ...
    val filteredDataArray = dataArray.filter(subtitle => asJavaStringMap(subtitle).get("SubFormat") == "srt")
    val sorted = filteredDataArray.sortWith { (o1, o2) =>
    ...
    val seasons: List[Season] =
      groupedBySeason
        .map { g =>
          ...
          //TODO improve this.
          val episode: (Int, String) = getSeriesEpisode(
            g._2.maxBy(m => getSeriesEpisode(m.asJava)._1).asJava
          )
  ...
  override def searchSubtitlesOnline(
    imdbId: String
  ): Option[SubtitleWithContent] = {
    val downloadWindow = DateTime.now.minusHours(HOURS_PER_DOWNLOAD_PERIOD)
    if (
      persistenceManager.subtitleDownloadsSince(
        downloadWindow
      ) >= MAX_DOWNLOADS_PER_PERIOD
    ) {
      val next = nextAvailableDownload
      logger.info(
        s"Too many subtitle downloads. Next download opportunity: $next"
      )
      None
    } else {
  ...
  override def nextAvailableDownload: DateTime = {
    persistenceManager.nextAvailableDownload(
      MAX_DOWNLOADS_PER_PERIOD,
      HOURS_PER_DOWNLOAD_PERIOD
    )
  }
  ...
  def downloadSubtitle(subtitleId: String, imdbId: String): Option[String] = {
    ...
    val response = client.execute(
      config,
      "DownloadSubtitles",
      Array[AnyRef](token, Array(subtitleId))
    )
    val responseMap = asAnyRefMap(response)
    responseMap.get("data") match {
      case bool: lang.Boolean if !bool =>
        ...
        logger.error(
          s"Something went wrong when downloading the subtitle with subId: $subtitleId.\n" +
            s"We might have exceeded the download limit: $status"
        )
        persistenceManager.saveDownloadError(
          subtitleId,
          imdbId,
          status.toString
        )
  ...
  private def performSearchOnline(
    imdbId: String
  ): Option[SubtitleWithContent] = {
    ...
    val optionalSubtitleMap =
      dataArray.toStream.map(asJavaStringMap).find { stringMap =>
        val subtitleId = stringMap.get("IDSubtitleFile")
        val shouldSkip = subtitleIdsToSkip.contains(subtitleId)
        if (shouldSkip) {
          logger.info(
            s"Skipping download of subtitle $subtitleId for imdbId $imdbId"
          )
        }
        !shouldSkip
      }
    optionalSubtitleMap.flatMap { stringMap =>
      val subtitleStringOption =
        downloadSubtitle(stringMap.get("IDSubtitleFile"), imdbId)
      subtitleStringOption.foreach(_ => persistenceManager.saveSubtitleDownload())
      val subtitleId = stringMap.get("IDSubtitle")
      subtitleStringOption.map(
        SubtitleWithContent(Subtitle(subtitleId, imdbId), _)
      )
    }
  }

  private def getSeriesEpisode(
    myMap: util.Map[String, String]
  ): (Int, String) = {
  ...
  private def subtitleResultsToMap(
    candidates: Array[Object],
    imdbId: String
  ): Array[immutable.Map[String, String]] = {
    try {
      candidates.map(asJavaStringMap(_).asScala.toMap)
    } catch {
      case NonFatal(e) =>
        logger.error(
          s"Unable to process open subtitle results for imdbId: $imdbId"
        )
        Array.empty[immutable.Map[String, String]]
    }
  }

  private def withTokenCheck(
    myToken: String
