...
object MovieAndSeriesDumpExporter extends OmdbDumpExporter {
  ...
  def exportMoviesWithImdbVotes(
    inputLocation: String,
    outputLocation: String
  ): Unit = {
    exportDumpInternal(inputLocation, outputLocation) { (values, indexByField) =>
      val optionalVotes = parseOptionalInt(values(indexByField(ImdbVotes)))
      val titleType = values(indexByField(Type))
      optionalVotes.exists(
        _ > 1000
      ) && (titleType == "movie" || titleType == "series")
    }
  }
}
...
class OmdbDumpExporter {
  def exportDump(inputLocation: String, outputLocation: String): Unit = {
    exportDumpInternal(inputLocation, outputLocation) { (_, _) =>
      true
    }
  }

  protected def exportDumpInternal(
    inputLocation: String,
    outputLocation: String
